"""Replace Buy high books + paytable rows from the certified math tables."""
...
def export_buy_mode(k: int, risk: str, paytable: list[float], buy: str, cost: float) -> dict:
    ...
    assert total == expect, f"high_pick_{k}_{buy} weight {total} != {expect}"
    rtp = sum(r["weight"] * r["payout"] for r in rows) / (total * 100 * cost)
    return {"cost": cost, "rtp": rtp, "books": rows}


def main() -> None:
    pulse = PULSE_BOOST["high"]
    for buy, cost in BUY_SUFFIXES.items():
        FRONT_PAY[buy]["high"] = PAY[buy]["high"]
        print(f"{buy}.high advertised tops", {k: max(v) for k, v in PAY[buy]["high"].items()})
        for k_s, row in PAY[buy]["high"].items():
            k = int(k_s)
            name = f"high_pick_{k}_{buy}"
            mode = export_buy_mode(k, "high", row, buy, cost)
