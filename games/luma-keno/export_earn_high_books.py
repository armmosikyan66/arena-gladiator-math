"""Replace Earn high books + paytable rows from the certified math tables."""
...
def export_earn_mode(k: int, risk: str, paytable: list[float]) -> dict:
    ...
    assert total == expect, f"high_pick_{k}_earn high pick {k} weight {total} != {expect}"
    rtp = sum(r["weight"] * r["payout"] for r in rows) / (total * 100)
    return {"cost": 1.0, "rtp": rtp, "books": rows}


def main() -> None:
    FRONT_PAY["earn"]["high"] = PAY["earn"]["high"]
    print(
        "earn.high advertised tops",
        {k: max(v) for k, v in PAY["earn"]["high"].items()},
    )
    for k_s, row in PAY["earn"]["high"].items():
        k = int(k_s)
        name = f"high_pick_{k}_earn"
        mode = export_earn_mode(k, "high", row)
