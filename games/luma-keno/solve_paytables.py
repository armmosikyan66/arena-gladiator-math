...
def planned_paying(
    risk: str, k: int, earn: bool = True, bought: bool = False
) -> frozenset[int]:
    if k == 1:
        return frozenset({0, 1})
    paying = set(range(PAY_START[risk][k], k + 1))
    rh = refund_hit(risk, k, earn, bought)
    if rh is not None:
        paying.add(rh)
    return frozenset(paying)


def settle_factor(risk: str, earn: bool) -> float:
    ...
    High. Off has neither channel, so advertised == settled and the caps apply
    directly. This one function is the whole difference between the two solves
    on the cap side.
    """
    return LUMEN_BOOST[risk] * PULSE_BOOST[risk] if earn else 1.0


def pay_coeff(risk: str, k: int, earn: bool = True, bought: bool = False) -> list[float]:
    ...
    return list(effective_coeff(risk, k, planned_paying(risk, k, earn), bought))


def advertised_cap(
    risk: str, k: int, h: int, earn: bool = True, cost: float = 1.0
) -> float:
    """Advertised ceiling so the settled prize stays under `cap_for`."""
    return (
        math.floor(
            cap_for(risk, k, h, cost) / settle_factor(risk, earn) * 10 + 1e-9
        )
        / 10
    )


def consolation_cap(risk: str, earn: bool = True) -> float:
    """Keep settled consolation under 40× so it does not load ETL40."""
    return math.floor(39.9 / settle_factor(risk, earn) * 10 + 1e-9) / 10


def mode_stats_for(
    risk: str, k: int, table: list[float], earn: bool, bought: bool = False
) -> dict:
    return settled_stats(risk, k, table, bought) if earn else base_stats(k, table)
...
def refund_hit(
    ...
    if earn:
        ...
        p = list(effective_coeff(risk, k, paying, bought))
    else:
        p = list(base_coeff(k))
    # Cheapest table that still honours the refund tier and monotone 0.1x steps.
    min_rtp = p[h] * REFUND_X + sum(
        p[t] * (REFUND_X + 0.1 * (t - start + 1)) for t in range(start, k + 1)
    )
...
def _fill_from(
    ...
    p = pay_coeff(risk, k, earn, bought)
    beta = RISK_SHAPES[risk]["beta"]
    start = PAY_START[risk][k]
    caps = {h: advertised_cap(risk, k, h, earn, cost) for h in range(start, k + 1)}
    ...
    sub40 = consolation_cap(risk, earn)
...
def solve_table(
    risk: str, k: int, earn: bool = True, bought: bool = False, cost: float = 1.0
    ...
    designed = _fill_from(risk, k, None, earn, bought, cost)
    ...
    if best[0] > RTP_TOL * 2:
        ...
        for i in range(steps):
            ...
            cand = _fill_from(risk, k, cand_pay, earn, bought, cost)
            if not _table_valid(cand, start, k, caps):
                continue
...
def buy_pick_one_row(risk: str) -> tuple[list[float], list[str]]:
    ...
    p = effective_coeff(risk, 1, paying, True)
    cap = advertised_cap(risk, 1, 1, True)
    best: tuple[float, list[float]] | None = None
    # Miss row stays under the hit row, and both stay on the legal 0.1x grid.
    for miss_i in range(0, int(round(cap * 10)) + 1):
        miss = round(miss_i * 0.1, 10)
        if p[0] * miss > RTP_TARGET:
            break
        hit = round((RTP_TARGET - p[0] * miss) / p[1] * 10) / 10
        if hit <= miss or hit > cap:
            continue
        table = [miss, hit]
        err = abs(sum(pi * mi for pi, mi in zip(p, table)) - RTP_TARGET)
        # Strict improvement only, and `miss` ascends, so ties keep the smallest
        # miss row. That is the intent: on a bought round any sub-cost miss pay
        # is an LDW, and bought extras already lift the hit rate enough that the
        # mode does not need a consolation tier to feel alive.
...
def _solve_ladder(
    earn: bool, bought: bool = False, suffix: str = "", cost: float = 1.0
    ...
    for risk in ("classic", "low", "medium", "high"):
        ...
        for k in PICKS:
            ...
            if k == 1:
                if bought:
                    table, errors = buy_pick_one_row(risk)
                else:
                    table = pick_one_row_earn(risk) if earn else pick_one_row(risk)
                    errors = []
            else:
                table, errors = solve_table(risk, k, earn, bought, cost)
            stats = mode_stats_for(risk, k, table, earn, bought)
...
def solve_buy() -> tuple[dict[str, dict[str, dict[str, list[float]]]], list[float]]:
    ...
    the purchase buys. Everything else (Lumen at the risk boost, Pulse, the cap
    ladder, the water-fill) is unchanged, so this reuses the Earn solve with
    `bought=True` and one substitution: the advertised rows come out in
    multiples of the *cost*, then get scaled to base-stake units on the way out.

    Why that scaling matters. The carried tables were denominated in base stake
    while the round cost 10x or 100x it, so the only thing making a buy round
    worth its price was `lumenBoost = cost`. That put 90-99% of the return in
    the Lumen channel: on the ~30% of rounds where the Lumen was not caught, a
    buy100 player got `m/100` of what they paid, which is nothing, no matter how
    many numbers they hit. Solving in cost units and boosting at the normal risk
    rate makes the base table carry the round again.
    ...
    for key, cost in BUY_COSTS.items():
        tables, ladder_rtps = _solve_ladder(
            earn=True, bought=True, suffix=f"_{key}", cost=cost
        )
        ladders[key] = {
            risk: {
