"""Regenerate all Buy100 modes: {risk}_pick_{1..10}_buy100 for all risks."""
...
if __name__ == "__main__":
    ...
    for risk in RISKS:
        for k in range(1, 11):
            paying = paying_from_table(config.keno_buy_paytable[BUY][risk][k])
            name = f"{risk}_pick_{k}_{BUY}"
            num_sim_args[name] = book_count_for_picks(
                k, paying=paying, bought=True, placed=lumen_placed_on_pick(BUY, k)
            )
    print("modes:", len(num_sim_args))
