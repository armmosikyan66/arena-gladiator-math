"""Regenerate every high-risk book/LUT: Off, Earn, buy10, buy100."""
...
if __name__ == "__main__":
    ...
    num_sim_args = {
        f"high_pick_{k}": len(off_outcomes(k, "high")) for k in range(1, 11)
    }
    for k in range(1, 11):
        paying = paying_from_table(config.keno_earn_paytable["high"][k])
        num_sim_args[f"high_pick_{k}_earn"] = book_count_for_picks(k, paying=paying)
        for buy in BUY_SUFFIXES:
            buy_paying = paying_from_table(config.keno_buy_paytable[buy]["high"][k])
            num_sim_args[f"high_pick_{k}_{buy}"] = book_count_for_picks(
                k,
                paying=buy_paying,
                bought=True,
